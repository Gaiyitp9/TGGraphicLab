...
namespace TG::Rendering::RayTracing
{
    ...
    public:
        Camera() = default;
        Camera(unsigned int imageWidth, Math::Vector3f cameraPosition, float aspectRatio, float focalLength,
    ...
    private:
        [[nodiscard]] Math::Geometry::Ray GetRay(unsigned int row, unsigned int column);
        ...
        float m_viewportWidth = 32.0f / 9.0f;
        ...
        Math::Vector3f m_viewportUpperLeft{
            m_position - Math::Vector3f(0.0, 0.0, m_focalLength) - 0.5 * (m_viewportU + m_viewportV)
        };
