...
namespace TG
{
    CubeExampleGL::CubeExampleGL(const std::weak_ptr<IDefaultVideoPort>& videoPort, const std::weak_ptr<ITimer>& timer)
        : m_camera(videoPort, timer),
        m_vertexShader("Shaders/GLSL/Cube.vert", ShaderStageGL::Vertex),
        m_fragmentShader("Shaders/GLSL/Cube.frag", ShaderStageGL::Fragment),
        m_geometryShader("Shaders/GLSL/Wireframe.geom", ShaderStageGL::Geometry)
    {
    ...
    void CubeExampleGL::Render()
    {
        ...
        glBindVertexArray(m_VAO);
        glBindProgramPipeline(m_pipeline);
        glPushDebugGroup(GL_DEBUG_SOURCE_APPLICATION, 0, -1, "Cubes");
