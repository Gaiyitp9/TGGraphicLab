...
namespace TG::Reflection
{
    ...
    TEST(TypeList, BasicOperations)
    {
        ...
        EXPECT_TRUE((std::is_same_v<decltype(typeList0.Get<0>())::Type, int>));

        TypeList<int> typeList1;
        EXPECT_TRUE((std::is_same_v<TypeList<int>::GetT<0>, int>));
        EXPECT_TRUE((std::is_same_v<decltype(typeList1.Get<0>())::Type, int>));

        EXPECT_TRUE((std::is_same_v<TypeList<int, float>::FirstT, int>));
        EXPECT_TRUE((std::is_same_v<decltype(typeList0.First())::Type, int>));
        EXPECT_TRUE((std::is_same_v<TypeList<int, float>::LastT, float>));
        EXPECT_TRUE((std::is_same_v<decltype(typeList0.Last())::Type, float>));
